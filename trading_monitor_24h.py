...
FINNHUB_KEY     = “d82qjjhr01qvkevnhji0d82qjjhr01qvkevnhjig”
...
SYMBOLS_FINNHUB = [(“QQQ”, “US100”), (“DIA”, “US30”), (“OANDA:EUR_JPY”, “EURJPY”)]
INTERVAL = “30min”
OUTPUTSIZE = 220
...
def fetch_finnhub(symbol: str, display_name: str, interval: str = “30”) -> Optional[pd.DataFrame]:
“”“Obtiene datos de Finnhub para US100, US30, EURJPY.”””
try:
import time as time_module
# Mapeo de intervalos
iv_map = {“5min”: “5”, “15min”: “15”, “30min”: “30”, “1h”: “60”, “4h”: “D”, “1day”: “D”}
res = iv_map.get(interval, “30”)

```
    # Calcular timestamps (300 velas hacia atras)
    now = int(time_module.time())
    mins = int(res) if res.isdigit() else 1440
    from_ts = now - (mins * 60 * 300)
    
    # Usar forex para EURJPY, stock para ETFs
    if "EUR" in symbol or "OANDA" in symbol:
    ...
    if data.get("s") != "ok":
        log.warning(f"{display_name} Finnhub: {data.get('s')}")
        return None
    ...
    df = df.dropna()
    log.info(f"{display_name}: {len(df)} velas obtenidas de Finnhub")
    return df
except Exception as e:
    log.error(f"Finnhub fetch error {display_name}: {e}")
    return None
```
...
fh_intervals = {“1h”: “60”, “4h”: “D”, “1day”: “D”}
for interval, label in intervals:
if source == “finnhub”:
fh_sym = [s for s, n in SYMBOLS_FINNHUB if n == display_name]
fh_sym = fh_sym[0] if fh_sym else symbol
df = fetch_finnhub(fh_sym, display_name, fh_intervals.get(interval, “60”))
else:
...
pts += 2 if aligned >= 3 else 1 if aligned >= 2 else 0
pts += 1 if vol else 0
...
ax_t.text(0.5, 0.20, f"Bull: {bull_n}/3", color=GREEN, fontsize=10,
          ha="center", va="top", transform=ax_t.transAxes)
ax_t.text(0.5, 0.12, f"Bear: {bear_n}/3", color=RED, fontsize=10,
          ha="center", va="top", transform=ax_t.transAxes)
...
f”Velas: 30M | Revision: 15min | Panel: 1H/4H/1D\n”
...
all_symbols = [(s, “twelve”, s.replace(”/”,””)) for s in SYMBOLS_TWELVE] + [(s, “finnhub”, n) for s, n in SYMBOLS_FINNHUB]
for symbol, source, display_name in all_symbols:
if source == “finnhub”:
fh_sym = [s for s, n in SYMBOLS_FINNHUB if n == display_name][0]
df = fetch_finnhub(fh_sym, display_name, “30”)
else:
